# Title: Simulation of environmental noise measurements for every census tract in the US by state for states without rail noise rasters
...
#Rail_raster <- ifelse(file.exists(rail_filename), raster(rail_filename, RAT = TRUE), NA)
#Rail_points <- ifelse(file.exists(rail_filename), rasterToPoints(Rail_raster, spatial = TRUE), NA) 
#Rail_points2 <- ifelse(file.exists(rail_filename), spTransform(Rail_points, CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0")), NA)
...
#rail_shp <- ifelse(file.exists(rail_filename), point.in.poly(x = Rail_points2, y = bg_shp3, sp = TRUE, duplicate = TRUE), NA)
...
#env_noise_2_final_df <- c()
all_simul <- c()
Iter <- 100

filename_simul <- paste0("/Users/keshavpatel/University of Michigan Dropbox/Keshav Patel/Noise OEJ/Data/Generated Data/Simulations/Environmental Simulated data/Environmental No Rail")
...
filename_agg_simul <- paste0("/Users/keshavpatel/University of Michigan Dropbox/Keshav Patel/Noise OEJ/Data/Generated Data/Final state aggregated data/Transportation/env_finaldata_", current_state, "_s", Iter, ".Rds")
...
#' colorado unimodal at >0
#' Colorado, Connecticut, Delaware, Idaho, Indiana, Maine have more variation (sd and mean-median)
